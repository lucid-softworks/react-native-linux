...
- **TurboModule manager + codegen** — third-party native modules can't autolink today. Lots of common libraries (FBSDK, MMKV, RNFS, sentry-react-native, …) need this. Blocks any RN ecosystem package with a `react-native.config.js`. **Design done in `docs/design-turbomodule-manager.md`** — four-phase plan, ~4-6 weeks. Existing scaffold (`TurboModuleRegistry`, `__turboModuleProxy`, autolink CLI) covers the basic plumbing; gap is codegen integration. Also the last remaining blocker for `shirakaba/expo-desktop` to drop in `expo-desktop-modules-core` as-is — until then, slot-in consumers fall back to the JS shim layer in `@lucid-softworks/react-native-linux-expo`.
