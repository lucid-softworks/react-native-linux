...
EOF
  ...
  if (( router )); then
    ...
    # ESM-form wrapper — esbuilds `define:` (in bundle.mjs) replaces
    # each `process.env.EXPO_PUBLIC_*` literal with a stub string at
    # bundle time, so module-eval-time env checks (with-convex throws
    # if EXPO_PUBLIC_CONVEX_URL is unset) pass without needing runtime
    # assignment ordering tricks.
    cat > "${wrapper}" <<EOF
