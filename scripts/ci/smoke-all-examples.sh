...
// The lucid-expo-router-routes import is a virtual module emitted by
// expoRouterRoutesPlugin in apps/playground/bundle.mjs — it walks the
// dir EXPO_ROUTER_APP_ROOT points at (set in the bundler env when this
// script invokes node bundle.mjs) and installs the routes table on
// globalThis. expo-router/entry then mounts the root layout.
process.env.EXPO_ROUTER_APP_ROOT = "${app_dir_abs}";
import "lucid-expo-router-routes";
...
EOF
  ...
  # EXPO_ROUTER_APP_ROOT is read at bundle time by the
  # expoRouterRoutesPlugin in bundle.mjs — it walks the dir, requires
  # every route file, and installs globalThis.__expoRouterRoutes so
  # the shim's <Stack>/<Slot> render real routes (not the placeholder).
  bundle_log="${ARTIFACT_DIR}/${name}.bundle.log"
  hbc_path="${REPO_ROOT}/apps/playground/linux/build/assets/index.linux.bundle.hbc"
  bundle_env=(RN_ENTRY_PATH="${REPO_ROOT}/${wrapper}")
  if (( router )); then
    bundle_env+=(EXPO_ROUTER_APP_ROOT="${app_dir_abs}")
  fi
  if ! env "${bundle_env[@]}" \
