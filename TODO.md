...
- **Real-app harness** — `build-playground-bundle` (commit ceeb0bdb) gates all 7 demo entries through esbuild + hermesc on every CI build; missing-`.hbc` is the signal for class-syntax / async / for-of regressions. **Still missing:** actually launching the binaries with `xvfb-run` + a simple "does the root view paint anything non-zero" check would catch C++-side mounting regressions the bundle gate can't.
