...
- **Real Fabric `EventEmitter` event dispatch** — native event handlers still call ad-hoc `dispatchFabric*` tag-registry helpers instead of `emitter->dispatchEvent(name, payload)`. Won't survive nested gestures. **Design done in `docs/design-fabric-event-emitter.md`** — three-phase plan. First-pass IdleEventBeat implementation got as far as `induce()` firing correctly but the JS handler never fired; the wiring gap between `RuntimeScheduler::scheduleWork` and the JS worker needs focused debugging before Phase 1 ships.
