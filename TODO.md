...
- **TurboModule manager + codegen** — Linux generator landed (`@lucid-softworks/react-native-linux-codegen`) covering primitives, void, objects/arrays via `folly::dynamic`, nullable, enum, and `Promise<T>` (JS-thread resolve). Wired into both `scripts/codegen/run.js` (in-tree) and `react-native autolink-linux` (third-party deps via the standard `codegenConfig` key). `PlatformConstants` is the live reference impl. Remaining gaps before arbitrary RN ecosystem packages drop in cleanly: (a) off-thread Promise resolution via `RuntimeExecutor`, (b) `Function` (callback) param types, (c) optional strongly-typed object structs in place of `folly::dynamic`. See `docs/expo-desktop-integration.md`.
