...
- [x] Codegen integration (TurboModules) — `@lucid-softworks/react-native-linux-codegen` emits real `Native*Spec.h` headers consumed by both in-tree and autolinked third-party deps; `PlatformConstants.cpp` is the reference impl. Fabric component generators (Props.h / ComponentDescriptors.h) still stub-only.
...
- **TurboModule manager + codegen** — Linux generator (`@lucid-softworks/react-native-linux-codegen`) covers primitives, void, typed C++ structs for any ObjectType (recursive, post-order, with `toDynamic` helpers), array / generic-object as `folly::dynamic`, nullable, enum, `Promise<T>` (off-thread safe via `RuntimeExecutor`), and `(...) => void` callback params (also via `RuntimeExecutor`). Wired into both `scripts/codegen/run.js` (in-tree) and `react-native autolink-linux` (third-party deps via the standard `codegenConfig` key). `PlatformConstants` is the live reference impl. Remaining nice-to-haves: typed `fromDynamic` deserialisation for object params, non-void callback returns, `schema.aliasMap` name reuse. See `docs/expo-desktop-integration.md`.
