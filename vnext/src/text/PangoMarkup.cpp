...
std::string spanAttrsFor(const facebook::react::TextAttributes& ta) {
  ...
  if (ta.foregroundColor) {
    ...
    if (a != 0xff) {
      std::snprintf(
          buf, sizeof(buf), " alpha=\"%u\"", static_cast<unsigned>((a * 65535u + 127u) / 255u));
      attrs += buf;
    }
  }

  if (!std::isnan(ta.fontSize) && ta.fontSize > 0) {
    std::snprintf(buf, sizeof(buf), " font_size=\"%dpt\"", static_cast<int>(ta.fontSize));
    attrs += buf;
  }
  ...
  if (ta.fontWeight) {
    char wbuf[24];
    std::snprintf(wbuf, sizeof(wbuf), " weight=\"%d\"", static_cast<int>(*ta.fontWeight));
    attrs += wbuf;
  }
