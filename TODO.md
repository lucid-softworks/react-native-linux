...
- Native Animated driver plumbed: `rnLinux.setNativeProp(nativeID, prop, value)` for `opacity` / `transform.translateX` / `transform.translateY` / `scale` / `scaleX` / `scaleY`. Uses `gtk_fixed_set_child_transform` (paint-only, no layout cascade) for translates. As of a1a84824 the AnimatedHost coalesces per-frame writes into a single `rnLinux.setNativeProps(id, bag)` call so a multi-prop transform (Paper's floating label rides translateX + translateY + scale together) rebuilds the GskTransform once per frame instead of three times. Currently NOT used by default — on this VM software paint dominates and React's implicit batching beats the per-listener fire of native. Will be a clear win on bare-metal GTK.
...
- ~~TurboModule manager + codegen~~ — landed in `@lucid-softworks/react-native-linux-codegen`. Covers every shape `@react-native/codegen`'s schema emits for both TurboModules and Fabric components, with `install<Impl>()` / `installComponent()` auto-registration, off-thread Promise + callback safety, schema.aliasMap dedup, and `react-native autolink-linux` driving per-dep codegen. See `docs/expo-desktop-integration.md`. (Lone edge: typed-object returns from sync callbacks — no real spec uses them.)
- ~~Fabric `EventEmitter` event dispatch~~ — landed end-to-end (Phases 1–3, `1f0a89da` / `adb853d5` / `3846ca02`). All 12 component events flow through the standard `eventEmitter_->dispatchEvent` pipeline driven by `IdleEventBeat`; the JS dispatcher walks the React fiber tree to invoke the first ancestor's `on<Name>` handler. PanResponder is the only event surface still on the legacy tag-registry path (responder-negotiation semantics).
...
- [x] Touch events synthesized from pointer — `GtkEventControllerLegacy` per `<View>` synthesizes onTouchStart / onTouchMove / onTouchEnd / onTouchCancel. Single-pointer (identifier=0) on desktop; payload mirrors RN's shape (`touches` + `changedTouches`, page + location coords, target tag). Bubbles through the standard EventEmitter pipeline.
- [x] Real Fabric `EventEmitter` plumbing — Phases 1–3 landed (`1f0a89da` / `adb853d5` / `3846ca02`). `IdleEventBeat` drives the chain via `g_idle_add`; `nativeFabricUIManager.registerEventHandler` walks the fiber tree from `instanceHandle` to invoke the first ancestor's `on<Name>`. All 12 component events (click, longPress, hoverIn/Out, focus/blur, submitEditing, keyPress, changeText, switchChange, scroll, refresh, layout) now flow through `eventEmitter_->dispatchEvent`. PanResponder stays on the legacy tag-registry path — the responder negotiation it implements isn't expressible as plain bubbling props.
...
### 5.6 — TurboModule + Fabric component codegen infrastructure

- [x] `TurboModuleRegistry` — process-wide singleton with `installTurboModuleBinding`; codegen-emitted `install<Impl>()` wires impls into it.
- [x] `LinuxComponentBootstrap` — process-wide accumulator for Fabric components; codegen-emitted `installComponent()` registers initializers that `makeLinuxComponentDescriptorRegistry()` applies to every host registry.
- [x] PlatformConstants returning Linux info — refactored to extend the codegen-generated `NativePlatformConstantsLinuxSpec` (`src/modules/PlatformConstants.cpp`).
- [x] AsyncStorage via JSI bindings (XDG JSON-on-disk, atomic save) — not a TurboModule yet.
- [x] Codegen integration (both TurboModules and Fabric components) — `@lucid-softworks/react-native-linux-codegen` emits real `Native*Spec.h` headers wired into the in-tree build via `scripts/codegen/run.js` and per-dep via `react-native autolink-linux`. Reference impls: `PlatformConstants.cpp` (TM) + `Switch.cpp` / `CodegenSmokeView.cpp` (components). Verified against `shirakaba/expo-desktop-modules-core`'s real specs.
...
- [x] **Location D-Bus setup** — landed in 7110f642. New `rnLinux.locationIsAvailableAsync(cb)` + `rnLinux.locationStartWatchAsync(onFix, onError, cb)` JSI bindings spawn `std::thread` for the blocking `g_bus_get_sync` + CreateClient/SetDesktopId/Start round-trip; hop back via `state().executor`. expo-location.js routes through async when present, sync fallback otherwise.
- [x] **AsyncStorage I/O** — landed in 61de0538. Background save thread coalesces writes; storage mutex held only for snapshot, released for disk I/O. `flushAsyncStorage()` drains the queue on shutdown via `RNLinuxApplication::onShutdown`. Cold-start `load()` pre-warm landed in 6a341122 — `prewarmAsyncStorage()` runs from the app ctor on a detached std::thread; first JS getItem no longer pays for the disk parse.
- [x] **Image `data:` URI decode** — landed in 00e41557. `extractDataUriPayload` (fast header parse) splits from `decodeDataUriPayload` (heavy decode); `kDataUriAsyncThreshold = 16 KB` routes large payloads to a detached `std::thread` that hops back via `g_idle_add(G_PRIORITY_HIGH_IDLE)`. Reuses the `rnl-current-uri` qdata slot the http path uses for supersession detection.
...
- [x] Inline `<Text>` styling — landed in 76551a0f. Outer `<Text>` emits a Paragraph; nested Text emits a 'Text' shadow node via an `InTextContext` React.Context branch in `components.js`. BaseTextShadowNode walks the descendants to build the AttributedString; PangoMarkup emits one `<span attrs…>` per fragment so multi-style runs render correctly.
...
- [x] `RefreshControl` — `GtkScrolledWindow::edge-overshot` (TOP) → `dispatchFabricRefresh(tag)` (dbe47c75). ScrollView shim extracts `refreshControl`'s `onRefresh` + `refreshing`; per-instance `refreshing_` gates re-fires so one gesture = one onRefresh.
...
- [x] `Animated.useNativeDriver` — `animated.js`'s `timing()` reads `config.useNativeDriver === true` and routes per-frame `setValue` through the native bypass for opacity + transform.translateX/Y/scale/scaleX/scaleY. `_nativeOnly` refcount on the source AnimatedValue suppresses React listeners while native is driving. Per-host coalesce (a1a84824): each AnimatedHost queues prop writes into a `pending` bag and flushes once per microtask via `rnLinux.setNativeProps(id, bag)` so a multi-prop transform rebuilds the GskTransform once per frame instead of per-prop.
...
3. ~~**Real Fabric EventEmitter**~~ — done (Phases 1–3, `3846ca02`). Bubble + capture + `stopPropagation` are the follow-up; today the fiber walk stops at the first matching ancestor (parity with the previous tag-registry behavior).
...
- **Real-app harness** — `build-playground-bundle` (commit ceeb0bdb) gates all 7 demo entries through esbuild + hermesc on every CI build; missing-`.hbc` is the signal for class-syntax / async / for-of regressions. `smoke-playground` (commit e9756380) takes it a step further — builds `rn-linux-playground`, bundles `index.tsx`, boots under `xvfb-run`, and asserts (a) process alive at screenshot time, (b) screenshot has ≥ 16 unique colours, and (c) the captured stderr contains `"JSX commit done"`. Triage artifacts (screenshot + app.log) upload on every run. The remaining gap is a _real third-party_ app (akari / a non-trivial Expo screen) running through the same gate — the trip-wire matters most when an unrelated landing breaks a workflow we don't author.
- **TurboModule + Fabric component codegen** — landed end-to-end in `@lucid-softworks/react-native-linux-codegen`. Every shape `@react-native/codegen`'s schema can emit lowers to typed C++: TM primitives / void / typed structs (with `schema.aliasMap` dedup, `toDynamic` + `fromDynamic` + ADL `fromRawValue`) / arrays / nullables / enums / `Promise<T>` off-thread / `(...) => void` callbacks off-thread / `(...) => primitive` callbacks sync-on-JS-thread / object args in callbacks / `install<Impl>()` registration; component primitives / Color / Point / EdgeInsets / Dimension / ImageSource / String + Int32 enums / object structs / `std::vector<T>` arrays / typed event payloads / `<Name>HandleCommand` dispatcher (runtime-wired via `LinuxComponentView::handleCommand` + `LinuxSchedulerDelegate::schedulerDidDispatchCommand`) / `installComponent()` auto-register via `LinuxComponentBootstrap`. Wired into `scripts/codegen/run.js` (in-tree) and `react-native autolink-linux` (third-party deps). Verified against `shirakaba/expo-desktop-modules-core`'s real specs. Compile-time guards: `CodegenSmoke.cpp` + `CodegenSmokeView.cpp`. See `docs/expo-desktop-integration.md` and `packages/@lucid-softworks/react-native-linux-codegen/README.md`.
- **Real Fabric `EventEmitter` event dispatch** — Phases 1–3 landed (`1f0a89da` + `adb853d5` + `3846ca02`). `IdleEventBeat` drives the chain via `g_idle_add`; `nativeFabricUIManager.registerEventHandler` walks the fiber tree from `instanceHandle` to invoke the first ancestor's `on<Name>`. All 12 component events (click, longPress, hoverIn/Out, focus/blur, submitEditing, keyPress, changeText, switchChange, scroll, refresh, layout) now flow through `eventEmitter_->dispatchEvent`. PanResponder is the lone holdout on the legacy tag-registry path — its responder-negotiation semantics aren't plain bubbling. Bubble + capture + `stopPropagation` are a follow-up; today the fiber walk stops at the first matching ancestor (parity with the old behavior). See `docs/design-fabric-event-emitter.md`.
- **Multi-window per running app** — one GtkApplicationWindow per process today; multi-window via `SurfaceHandler` inside the same Hermes runtime is Phase 3 of the multi-instance plan. Optional for shipping single-window apps. **Design done in `docs/design-multi-instance.md`**. Phase 1 (per-binary application-id) + Phase 2 (per-app sandbox paths across AsyncStorage / SecureStore / KeepAwake / Notifications / FileSystem / DeviceInfo / Camera / Location / Image cache) landed in df65f00a / 2e70b878 / 345847b6 / 93b070de.
- **`expo-modules-core` registry** — shim lives at `packages/@lucid-softworks/react-native-linux-expo/expo-modules-core.js` (commit a2cb0c21). `requireNativeModule(name)` looks up `globalThis.expo.modules[name]`; in-tree shims that want to surface as expo-modules-core native modules call `registerExpoModule(name, impl)`. Third-party Expo packages from npm resolve via the same path once their canonical name is registered.
...
- **Distro packaging** — AppImage script, `.deb` packager, and `.rpm` packager (landed 8112132a) all exist; CLI exposes them via `react-native pack-linux --target=deb|rpm|appimage` (defaults pulled from `package.json`). Flatpak / Snap still open — each ~50–100 lines of script following the same shape as `scripts/package/deb.sh`. See `docs/packaging.md`.
