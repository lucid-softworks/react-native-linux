...
- [x] **AsyncStorage I/O** — landed in 61de0538. Background save thread coalesces writes; storage mutex held only for snapshot, released for disk I/O. `flushAsyncStorage()` drains the queue on shutdown via `RNLinuxApplication::onShutdown`. Cold-start `load()` still blocks first read; pre-warm from host start is a small follow-up.
