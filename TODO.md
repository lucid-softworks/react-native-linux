...
- [x] `Animated.useNativeDriver` — `animated.js`'s `timing()` reads `config.useNativeDriver === true` and routes per-frame `setValue` straight through `rnLinux.setNativeProp` (bypassing the React listener path) for opacity + transform.translateX/Y. `_nativeOnly` refcount on the source AnimatedValue gates the bypass. Per-frame batching (multiple property writes flush as one GTK invalidation) is the follow-up perf knob, not a correctness gap.
