...
- [/] Real Fabric `EventEmitter` plumbing — Phase 1 (`IdleEventBeat` + `nativeFabricUIManager.registerEventHandler`) + Phase 2 (click migrated) landed in `adb853d5` / `1f0a89da`. Phase 3 migrates the remaining 11 events (longPress, hoverIn/Out, focus/blur, submitEditing, keyPress, changeText, switchChange, scroll, layout).
...
3. **Real Fabric EventEmitter** — replace the dispatchFabric\* JSI tag-registry with the EventEmitter::dispatchEvent path so nested gestures route correctly through the standard React event system.
...
- **Real Fabric `EventEmitter` event dispatch** — Phase 1 + 2 landed (`1f0a89da` + `adb853d5`). `IdleEventBeat` drives the chain via `g_idle_add`, `nativeFabricUIManager.registerEventHandler` walks the fiber tree from instanceHandle to dispatch to the first ancestor's `on<Name>`. Click is migrated. Phase 3 (the remaining 11 events: longPress, hoverIn/Out, focus/blur, submitEditing, keyPress, changeText, switchChange, scroll, layout) is mostly mechanical — each maps a `dispatchFabric<X>` call to `emitter->dispatchEvent` and drops the matching `sync<X>Handler` from `fabricHostConfig.js`. See `docs/design-fabric-event-emitter.md`.
